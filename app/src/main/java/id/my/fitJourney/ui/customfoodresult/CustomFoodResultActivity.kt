...
class CustomFoodResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.searchViewCustomFood.setOnQueryTextListener(object : androidx.appcompat.widget.SearchView.OnQueryTextListener {
    ...
    private fun setFoodsData() {
        ...
        // PERTAHANKAN LOGIKA isFavoriteMap SEPERTI YANG ASLI
        ...
        viewModel.search.observe(this, { value ->
            if (value != null && value.isNotEmpty()) {
                val modified =
                    foodData?.filter { food ->
                        food.name?.lowercase()?.contains(value.lowercase()) ?: false
                    }
                        ?: emptyList()
        ...
        adapter.setOnItemClickCallback(object : CustomFoodAdapter.OnItemClickCallback {
            override fun onItemClicked(data: CustomFoodResponseItem) {
                ...
                intent.putExtra(DetailFoodActivity.EXTRA_TYPE, type)
                startActivity(intent)
            }
            ...
            override fun onFavoriteClicked(data: CustomFoodResponseItem, btnFavorite: ImageButton) {
                ...
                data.name?.let {
                    ...
                    // Update status favorit berdasarkan isFavoriteMap yang ada
                    viewModel.isFoodFavorite(it).observe(this@CustomFoodResultActivity) { number ->
                        isFavorite = number > 0
                    }

                    // Menyimpan atau menghapus makanan dari favorit
                    if (isFavorite) {
